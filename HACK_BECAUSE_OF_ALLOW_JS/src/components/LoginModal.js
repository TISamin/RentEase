...
export function LoginModal() {
    ...
    return (<Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[400px]">
        ...
        {mode === "login" ? (<form onSubmit={handleLogin} className="space-y-4 pt-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={() => { setMode("forgot_password"); setSignupStep("email"); setPasswordState(""); }} className="text-xs text-primary hover:underline font-medium focus:outline-none">
                  Forgot password?
                </button>
              </div>
              <Input id="login-password" type="password" placeholder="••••••••" value={password} onChange={(e) => setPasswordState(e.target.value)} required/>
            </div>
            ...
            <div className="text-center mt-4">
              ...
              <button type="button" onClick={() => { setMode("signup"); setSignupStep("email"); setPasswordState(""); }} className="text-sm font-medium hover:underline text-primary">
                Sign up
              </button>
            </div>
          </form>) : (<div className="pt-4">
            {signupStep === "email" && (<form onSubmit={handleSendOtp} className="space-y-4">
                <div className="space-y-2">
            ...
            {signupStep === "otp" && (<form onSubmit={handleVerifyOtp} className="space-y-6">
            ...
            {signupStep === "password" && (<form onSubmit={handleSetPassword} className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="signup-password">Create Password</Label>
                  <Input id="signup-password" type="password" placeholder="Minimum 6 characters" value={password} onChange={(e) => setPasswordState(e.target.value)} required autoFocus/>
                </div>
            ...
            {signupStep === "email" && (<div className="text-center mt-6">
                <span className="text-sm text-muted-foreground">
                  {mode === "forgot_password" ? "Remember your password? " : "Already have an account? "}
                </span>
                <button type="button" onClick={() => setMode("login")} className="text-sm font-medium hover:underline text-primary">
                  Log in
                </button>
              </div>)}
          </div>)}

...
import { Building2, PlusCircle, Search, SlidersHorizontal, Briefcase } from "lucide-react";
import { useCallback, useEffect, useState } from "react";
import { ListingCard, ListingCardSkeleton } from "../components/ListingCard";
import { useBrowseListings } from "../hooks/useBackend";
import type { BrowseFilter } from "../types";
import { SERVICE_CATEGORIES, CATEGORY_LABELS, Category } from "../types";
...
export default function Services() {
  ...
  const selectedCategory = categoryValue !== "all" ? (categoryValue as Category) : undefined;
  const filter: BrowseFilter = { locationQuery: debouncedLocation.trim() || undefined, category: selectedCategory };
  ...
  const handleClearFilters = useCallback(() => { setLocationInput(""); setCategoryValue("all"); }, []);
        ...
        <div className="relative container mx-auto px-4 sm:px-6 py-8 sm:py-10">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-display text-2xl sm:text-3xl font-bold text-foreground leading-tight">Services Directory</h1>
                <p className="text-muted-foreground text-sm sm:text-base mt-0.5">Find event planning, catering, decoration, and maintenance services near you.</p>
              </div>
            </div>
            <Link to="/post-listing">
              <Button className="gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white shadow-md hover:shadow-lg transition-all duration-300">
                <PlusCircle className="h-4 w-4" />Provide Service
              </Button>
            </Link>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 max-w-3xl">
            <div className="relative flex-1">
              ...
              <Input data-ocid="browse.location.search_input" type="search" placeholder="Search by city, area, or location…" value={locationInput} onChange={(e) => setLocationInput(e.target.value)} className="pl-9 bg-background/80 backdrop-blur-sm border-input/60 focus-visible:ring-blue-500/50" aria-label="Search by location" />
            </div>
            <div className="sm:w-52 shrink-0">
              <Select value={categoryValue} onValueChange={setCategoryValue}>
                <SelectTrigger data-ocid="browse.category.select" className="bg-background/80 backdrop-blur-sm border-input/60 w-full" aria-label="Filter by category">
                ...
                <SelectContent>
                  <SelectItem value="all" data-ocid="browse.filter.all">All categories</SelectItem>
                  {SERVICE_CATEGORIES.map((cat) => (
                    <SelectItem key={cat} value={cat} data-ocid={`browse.filter.${cat}`}>{CATEGORY_LABELS[cat]}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
      ...
      <section className="container mx-auto px-4 sm:px-6 py-8">
        ...
        {!isLoading && !isError && listings?.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 gap-5 text-center" data-ocid="browse.empty_state">
            <div className="rounded-2xl bg-gradient-to-br from-blue-500/10 to-indigo-500/10 p-5 shadow-inner"><Briefcase className="h-10 w-10 text-blue-500/50" aria-hidden="true" /></div>
            <div className="flex flex-col gap-1">
              <h2 className="font-display font-semibold text-foreground text-lg">No services found</h2>
              <p className="text-muted-foreground text-sm max-w-xs">{hasActiveFilters ? "Try adjusting your location search or changing the category filter." : "No services are available right now. Check back soon."}</p>
            </div>

...
export function useBrowseListings(filter: BrowseFilter) {
...
export function useGetListing(id: ListingId | null) {
  return useQuery<ListingPublic | null>({
    queryKey: ["listing", id],
    queryFn: async () => {
...
export function useUpdateListing() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, input }: { id: ListingId; input: any }) => {
      return fetchWithAuth(`/listings/${id}`, {
        method: "PUT",
        body: JSON.stringify(input)
      });
